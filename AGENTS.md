...
Run the app locally with `nix develop`, `just install`, and `just dev`. On a `noexec` project volume use `just install-tmp`. `just build` rebuilds the browser index and `dist/` without reading `response/*.json`; `just test` and `just verify-data` are the automated checks. Install data-pipeline dependencies with `pip install -r requirements.txt` only when refreshing local source data. If legacy scripts regenerate raw API responses, keep them local-only under ignored `response/*.json`.
...
## Testing Guidelines
Use `just test`, `just verify-data`, and `just build` as the baseline automated checks. After frontend edits, also smoke-test the built site in a browser at desktop and mobile viewport sizes.
