...
foreach ($file in $files) {
    try {
        ...
        # 5. Length limit WITHOUT truncation dots - just keep the first sentence if too long
        if ($clean.Length -gt 45) {
            $firstSentence = $clean.Split("。")[0]
            if ($firstSentence.Length -gt 44) {
                $clean = $firstSentence.Substring(0, 44) + "。"
            } else {
                $clean = $firstSentence + "。"
            }
        }
...
Write-Host "Summaries updated (v3 - no dots)."