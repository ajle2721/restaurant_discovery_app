...
# 強制使用帶 BOM 的 UTF8 讀取，避免亂碼
$aiFiles = Get-ChildItem $aiReviewDir -Filter "*.json" | Sort-Object Name
$records = @()
Write-Host "🚀 正在使用 PowerShell 執行終極打包 (含農人餐桌修復)..."
foreach ($file in $aiFiles) {
    ...
    if (Test-Path $responsePath) {
        try {
            ...
            # 關鍵修正：地址同時處理 臺/台 轉換與 簡體 区/區 轉換
            $address = if ($response.formattedAddress) { $response.formattedAddress.Replace("臺", "台").Replace("区", "區") } else { "" }
            $district = ""
            foreach ($d in $taipeiDistricts) { if ($address.Contains($d)) { $district = $d; break } }
            if ($address.Contains("中正")) { $district = "中正區" }
...
Write-Host "✨ 打包大功告成！已處理 $($records.Count) 間餐廳。"